...
// GET - Fetch fluid intake records for a specific date
export async function GET(request: Request) {
  try {
    ...
    const records = await prisma.fluidIntakeRecord.findMany({
      where: {
    ...
    console.error("Fetch fluid intake records error:", error)
    return NextResponse.json(
...
// POST - Create fluid intake record
export async function POST(request: Request) {
  try {
    ...
    const { clientId, recordDate, recordTime, volume, fluidType, notes } = body
    ...
    const record = await prisma.fluidIntakeRecord.create({
      data: {
        clientId,
    ...
    console.error("Create fluid intake record error:", error)
    return NextResponse.json(
      { error: "Er is een fout opgetreden" },
      { status: 500 }
    )
...
// DELETE - Delete fluid intake record
export async function DELETE(request: Request) {
  try {
    ...
    const record = await prisma.fluidIntakeRecord.findUnique({
      where: { id: recordId },
    })
    ...
    await prisma.fluidIntakeRecord.delete({
      where: { id: recordId },
    })

    return NextResponse.json({ message: "Record verwijderd" })
  } catch (error) {
    console.error("Delete fluid intake record error:", error)
    return NextResponse.json(
      { error: "Er is een fout opgetreden" },
      { status: 500 }
    )
